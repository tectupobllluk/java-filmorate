...
public class BaseUserService implements UserService {
    ...
    public User getUser(long userId) {
        return userRepository.getUser(userId).orElseThrow(() -> new NotFoundException("User not found with id = " + userId));
    }

    @Override
    public void addFriend(long friendId, long userId) {
        User user = userRepository.getUser(userId).orElseThrow(() -> new NotFoundException("User not found with id = " + userId));
        User friend = userRepository.getUser(friendId).orElseThrow(() -> new NotFoundException("Friend not found with id = " + friendId));
        feedService.createFeed(userId, FeedTypeEnum.FRIEND, FeedOperationEnum.ADD, friendId);
    ...
    public void deleteFriend(long friendId, long userId) {
        User user = userRepository.getUser(userId).orElseThrow(() -> new NotFoundException("User not found with id = " + userId));
        User friend = userRepository.getUser(friendId).orElseThrow(() -> new NotFoundException("Friend not found with id = " + friendId));
        feedService.createFeed(userId, FeedTypeEnum.FRIEND, FeedOperationEnum.REMOVE, friendId);
